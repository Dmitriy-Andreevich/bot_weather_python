...
def create_weather_image(req):
    ...
    paintLayer = ImageDraw.Draw(background)
    ...
    for day in req:
        for el in day:
            ...
            if req.__len__() > 1 and datetime.fromtimestamp(el['dt']).hour != 12:
                ...
                paintLayer.text((bottom_element_start_x_coord_local + BOTTOM_ELEMENT_TEMP_OFFSET_X_CORD,
                ...
                background.paste(cl, (
                    ...
                    BOTTOM_ELEMENT_START_Y_COORD + BOTTOM_ELEMENT_SECOND_LINE_OFFSET_Y_CORD),
                                 cl)
                paintLayer.text((bottom_element_start_x_coord_local + BOTTOM_ELEMENT_DATETIME_OFFSET_X_CORD,
    ...
    paintLayer.text((TOP_ELEMENT_TEMP_X_COORD, TOP_ELEMENT_TEMP_Y_COORD),
                    get_modify_temp(int(first_item_value[0]['main']['temp'])) + "°C",
                    fill=(255, 255, 255), font=TOP_TEMP_FONT)
    paintLayer.text((TOP_ELEMENT_DESC_X_COORD, TOP_ELEMENT_DESC_Y_COORD),
                    str(first_item_value[0]['weather'][0]['description']),
                    fill=(255, 255, 255), font=TOP_DESC_FONT)
    paintLayer.text((TOP_ELEMENT_DESC_X_COORD, TOP_ELEMENT_DESC_Y_COORD + MIN_TEMP_Y_OFFSET),
                    "Минимальная температура: " + str(min(temps)) + "°C",
                    fill=(255, 255, 255), font=BOTTOM_TIME_FONT)
    paintLayer.text((TOP_ELEMENT_DESC_X_COORD, TOP_ELEMENT_DESC_Y_COORD + MAX_TEMP_Y_OFFSET),
                    "Максимальная температура: " + str(max(temps)) + "°C",
                    fill=(255, 255, 255), font=BOTTOM_TIME_FONT)
    paintLayer.text((TOP_ELEMENT_DESC_X_COORD, TOP_ELEMENT_DESC_Y_COORD + BAR_Y_OFFSET),
                    "Давление: " + str(int(first_item_value[0]['main']['pressure'])) + " bar",
                    fill=(255, 255, 255), font=BOTTOM_TIME_FONT)
    paintLayer.text((TOP_ELEMENT_DESC_X_COORD, TOP_ELEMENT_DESC_Y_COORD + WIND_Y_OFFSET),
